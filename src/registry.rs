...
impl PortRegistry {
    ...
    pub fn new(
        ...
        let store_path = data_dir.join(PathBuf::from("registry.toml"));
        ...
        let registry = Self {
            store_path,
            gallery_www_path: data_dir.join(PathBuf::from("gallery_www")),
    ...
    pub fn caddyfile(&self) -> String {
        ...
        let gallery = format!(
            "localhost {{\n\tfile_server {{\n\t\troot \"{}\"\n\t}}\n}}\n\n",
            self.gallery_www_path.to_string_lossy()
        );
    ...
    // Return the generated gallery index.html
    pub fn gallery_index(&self) -> String {
        let projects = self
            .projects
            .iter()
            .map(|(name, project)| {
...
"#,
    ...
    pub fn reload_caddy(
        ...
        let gallery_index_path = self.gallery_www_path.join(PathBuf::from("index.html"));
        deps.write_file(&gallery_index_path, &self.gallery_index())
            .with_context(|| {
                format!(
